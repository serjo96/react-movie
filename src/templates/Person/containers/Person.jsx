...
class Person extends Component {
  ...
  render () {
    const person = this.props.people.data;
    const { imgIndex } = this.state;

    const lastMovies = this.props.people.sortedMovies.concat().filter(a => new Date(a.release_date) < new Date()).splice(0, 3);
    const lastTV = this.props.people.sortedTV.concat().filter(a => new Date(a.first_air_date) < new Date()).splice(0, 3);
    const bestMovies = person.movie_credits.cast.concat().filter(a => new Date(a.release_date) < new Date()).sort((a, b) => b.vote_average - a.vote_average).splice(0, 3);
    const bestTV = person.tv_credits.cast.concat().sort((a, b) => b.vote_average - a.vote_average).splice(0, 3);
    const years = new Date().getFullYear() - new Date(person.birthday).getFullYear();
    return (
