			...
			<aside className="aside ">
					...
					<div className="genres__list">
						{this.props.genres.map(( el, indx ) =>
							<div className='genre' key={indx}>
								<Link to={`/lists/genres_tv/${urlRusLat(el.name)}-${el.id}`} className="tag"
								      id={el.id}>{el.name}</Link>
							</div>
						)}
					</div>
				</div>
				...
				<div className="keywords infoTable-row">
					...
					<div className="keywords__list">
						{this.props.keywords.map(( el, indx ) => <Link
							...
							id={el.id} key={indx}>{el.name}</Link>)}
					</div>
				</div>
				...
				<div className="movie-links infoTable-row">
					...
					<div className="movie-links__list">
						{this.props.links.imdb_id ? <a href={'http://www.imdb.com/title/' + this.props.links.imdb_id} target='_blank'
						                       className='social-link'>imdb</a> : null}
						{this.props.links.tvdb_id ?
							<a href={'https://www.thetvdb.com/?tab=series&id=' + this.props.links.tvdb_id} target='_blank'
							   className='social-link'>tvdb</a> : null}
						<a href={'https://www.themoviedb.org/tv/' + this.props.id} target='_blank'
						   className='social-link'>TMDB</a>
						{this.props.homepage ?
