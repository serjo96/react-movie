			...
			<aside className="aside">
						...
						<div className="crew__names aside-row__right-col">
							{this.props.crew.Director.map(( men, indx ) => indx < 3 ?
								<div className="crew__name" key={indx}><Link
									...
									className="link">{men.name}</Link></div> : null)}
						</div>
					</div>
					...
					<div className="genres__list">
						{this.props.genres.map(( el, indx ) =>
							<div className='genre' key={indx}>
								<Link to={`/lists/genres_movie/${urlRusLat(el.name)}-${el.id}`} className="tag"
				...
				<div className="movie-links infoTable-row">
					...
					<div className="movie-links__list">
						{this.props.imdb_id ? <a href={'http://www.imdb.com/title/' + this.props.imdb_id} target='_blank'
						                    className='social-link'>imdb</a> : null}
						<a href={'https://www.themoviedb.org/movie/' + this.props.id} target='_blank'
						   className='social-link'>TMDB</a>
						{this.props.homepage ?
							<a href={this.props.homepage} target='_blank' className='social-link'>Страница фильма</a> : null}
					</div>
				</div>
